...
class RealSSHService: NSObject, SSHService {
    ...
    private var shell: SSHShell? = nil
    ...
    func openPTYShell(_ onOutput: @escaping (String) -> Void) async throws -> (String) -> Void {
        ...
        // Open a shell with PTY
        shell = try await client.openShell(terminal: term)
        
        // Start reading output from the shell
        Task {
            do {
                while let data = try await shell?.readData() {
                    if let output = String(data: data, encoding: .utf8) {
                        onOutput(output)
                    }
                }
            } catch {
                print("[RealSSHService] Shell read error: \(error)")
                onOutput("\n[Shell closed: \(error)]")
            }
        }
    ...
    private func sendToShell(_ input: String) {
        print("[RealSSHService] Sending to shell: \(input)")
        guard let shell = shell else {
            print("[RealSSHService] No shell available")
            return
        }
